...
/// `{success, data, error, meta}` 응답 바디를 언래핑한 결과.
class ApiEnvelope {
...
///   페이지네이션 `meta`(total 등)가 필요한 호출은 [getWithMeta]를 쓴다.
...
class ApiClient {
  ...
  Future<dynamic> get(String path, {Map<String, dynamic>? queryParameters}) async {
    return (await _request('GET', path, queryParameters: queryParameters)).data;
  }

  Future<ApiEnvelope> getWithMeta(
    String path, {
  ...
  Future<dynamic> post(String path, {dynamic data}) async {
    return (await _request('POST', path, data: data)).data;
  }

  Future<dynamic> put(String path, {dynamic data}) async {
    return (await _request('PUT', path, data: data)).data;
  }

  Future<dynamic> delete(String path, {dynamic data}) async {
    return (await _request('DELETE', path, data: data)).data;
  }
  ...
  Future<bool> _tryRefresh() async {
    ...
    try {
      ...
      final result = unwrapEnvelope(response.data).data as Map<String, dynamic>;
...
/// `{success, data, error, meta}` 응답 바디를 언래핑한다. 성공이면 [ApiEnvelope]를,
/// 실패면 [PiumApiException]을 던진다. Dio 의존성이 없는 순수 함수라
/// [ApiClient] 없이도 단위테스트할 수 있다.
ApiEnvelope unwrapEnvelope(dynamic body) {
  if (body is! Map) {
